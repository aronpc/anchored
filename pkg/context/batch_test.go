...
func newBatchTestEnv(t *testing.T) (*BatchExecutor, *sql.DB) {
	t.Helper()
	dsn := "file:" + t.Name() + "_batch?mode=memory&cache=private&_busy_timeout=5000"
...
func TestBatchExecutor_WithSearch(t *testing.T) {
	...
	result, err := be.ExecuteBatch(ctx, cmds, queries, "sess-batch-4", "", "", 0)
	if err != nil {
		t.Fatalf("ExecuteBatch: %v", err)
	}
...
func TestBatchExecutor_EmptyCommands(t *testing.T) {
	...
	result, err := be.ExecuteBatch(ctx, nil, nil, "sess-batch-5", "", "", 0)
	if err != nil {
		t.Fatalf("ExecuteBatch: %v", err)
	}
